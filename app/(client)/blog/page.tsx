...
import { AllBlogCategories, Container } from '@/components/index';
import { ALL_BLOGS_CATEGORIES_QUERYResult, ALL_BLOGS_QUERYResult, LATEST_BLOGS_QUERYResult} from '@/sanity.types';
import { urlFor } from '@/sanity/lib/image';
import { getAllBlogs, getAllBlogsCategories, getLatestBlogs } from '@/sanity/queries/sanity.index';
    ...
    <Container className="py-[106px]">
      <div className="flex flex-col md:flex-row gap-12 md:gap-8 min-h-screen">
        <div className=" w-full flex-2 space-y-5">
          {blogs &&
            blogs.map((blog) => (
              <div key={blog._id} className=' shadow-[0_0_35px_0px] bg-brown_light p-4 rounded-2xl'>
                <Image
                  src={
                    blog.mainImage
                ...
                <div className="flex gap-4 md:gap-8 text-sm md:text-[16px] text-gray-light my-4">
                  ...
                  <span className="flex items-center gap-2">
                    <FaTag />
                    <span>{blog.categories && blog.categories.join(', ')}</span>
                  </span>
                </div>
                ...
                <div className='mt-7 mb-5 relative w-fit group cursor-pointer'>
                  <Link href={`/blog/${blog.slug?.current}`} className='group-hover:text-black/60'>Read more</Link>
